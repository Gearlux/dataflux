...
from dataflux.storage.zarr import ZarrGroupSink, ZarrBatchSink
...
def test_directory_storage(tmp_path: Path) -> None:
    ...
    assert (dir_path / "000000" / "sample.npz").exists()
    assert (dir_path / "000001" / "sample.npz").exists()
...
def test_zarr_group_with_target(tmp_path: Path) -> None:
    ...
    assert "target" in z["sample_000000"]
